...
import { Trash2, Edit2, Plus, BarChart3, Calendar, Zap, ArrowLeft } from 'lucide-react';
...
export default function AdminResidentReadingsPage() {
  const params = useParams();
  const residentId = parseInt(params.residentId as string);
  const { user } = useAuth();
  ...
  useEffect(() => {
    fetchAdminResidentReadings(residentId);
  }, [residentId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      await updateAdminReading(residentId, editingId, data);
      setEditingId(null);
    } else {
      await createAdminReading(residentId, data);
    }
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Êtes-vous sûr de vouloir supprimer cette lecture?')) {
      await deleteAdminReading(residentId, id);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-4">
        <Link href="/admin/residents" className="p-2 hover:bg-white/10 rounded-lg transition">
          <ArrowLeft size={24} className="text-slate-400" />
        </Link>
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight">
            Lectures du Résident
          </h1>
          <p className="text-slate-500 mt-2">
            Gestion des lectures de consommation pour ce résident
          </p>
        </div>
      </div>
